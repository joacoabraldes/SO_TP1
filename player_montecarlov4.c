...
/*
 * player_montecarlov2_fixed.c
 *
 * Corrected & improved MonteCarlo v2 player. Fixes compilation errors and
 * logic bugs introduced previously (typos in function signatures and a
 * bracket mistake). Keeps the anti-suicide heuristics and adds a small
 * safety when picking among candidates.
 */
...
static int sim_pick_policy_move(int *board,int width,int height,sim_player_t *players,int player_count,int pid){
    ...
    for(int d=0; d<8; d++){
        ...
        if(tx<0||tx>=width||ty<0||ty>=height) continue;
        int cell = board[ty*width+tx]; if(cell<=0) continue;
    ...
    for(int i=0;i<valid_count;i++){
        ...
        /* temporarily mark as occupied to evaluate liberties */
        ...
        /* penalize immediate-suicide moves heavily */
...
static void compute_voronoi_potential_buf(int *board,int width,int height,sim_player_t *players,int player_count,unsigned int *vor_out,int *dist,int *owner,int *qx,int *qy,int *qo){
    ...
    while(qh<qt){
        ...
        int base=y*width+x; int dcur=dist[base];
    ...
    for(int i=0;i<n;i++){
        ...
        int o = owner[i]; if(o>=0) vor_out[o] += (unsigned int)board[i];
    }
}
...
int main(int argc,char *argv[]){
    ...
    while(1){
        ...
        for(int d=0; d<8; d++){
            ...
            int cell=board_snapshot[ty*gwidth+tx]; if(cell<=0) continue;
            valid_dirs[valid_count]=d; immediate_vals[valid_count]=cell; valid_count++;
        }
        ...
        if(free_cells >= opening_threshold){ /* cheap pick */
            ...
            for(int i=0;i<valid_count;i++){
                int d=valid_dirs[i]; int tx,ty; target_from_dir(gx,gy,d,&tx,&ty);
                int neigh_sum=0; for(int dd=0;dd<8;dd++){ int nx,ny; target_from_dir(tx,ty,dd,&nx,&ny); if(nx<0||nx>=gwidth||ny<0||ny>=gheight) continue; int v=board_snapshot[ny*gwidth+nx]; if(v>0) neigh_sum+=v; }
                double val = (double)immediate_vals[i] + 0.25*(double)neigh_sum;

                /* compute liberties after making this move (penalize suicides) */
                ...
                if(lib == 0) val -= 1000.0; /* large penalty to avoid suicide */

                if(val>bestv){ bestv=val; bc=0; bests[bc++]=d; } else if(val==bestv) bests[bc++]=d;
            }
            ...
            unsigned char mv=(unsigned char)pick; ssize_t w=write(STDOUT_FILENO,&mv,1);
            sem_post(&game_sync->state_mutex);
        ...
        if(bestc2>1){ double best_comb = -DBL_MAX; int topk = bestc2; if(topk>4) topk=4; for(int t=0;t<topk;t++){ int cand = bests2[t]; copy_board(board_sim, board_snapshot, gwidth*gheight); memcpy(players_sim, players_snapshot, sizeof(sim_player_t)*gplayer_count); sim_apply_move(board_sim,gwidth,gheight,players_sim,gplayer_count,my_index,cand); compute_voronoi_potential_buf(board_sim,gwidth,gheight,players_sim,gplayer_count,vor_tmp,dist,owner,qx,qy,qo); double my_vor = (double)vor_tmp[my_index]; double gamma = 0.03; double avg = candidate_avgs[t]; double combined = avg + gamma * my_vor; if(combined>best_comb){ best_comb=combined; pick=cand; } } }

        /* Anti-suicide final check: if selected move leaves zero liberties try to pick alternative */
        {
            int tx,ty; target_from_dir(gx,gy,pick,&tx,&ty);
            int saved = board_snapshot[ty*gwidth+tx];
            int oldx = players_snapshot[my_index].x, oldy = players_snapshot[my_index].y;
            board_snapshot[ty*gwidth+tx] = -(my_index+1);
            players_snapshot[my_index].x = tx; players_snapshot[my_index].y = ty;
            ...
            board_snapshot[ty*gwidth+tx] = saved;
            if(lib == 0){
                int best_non_suicide = -1; double best_val = -DBL_MAX;
                for(int i=0;i<valid_count;i++){
                    int d = valid_dirs[i]; int tx2,ty2; target_from_dir(gx,gy,d,&tx2,&ty2);
                    ...
                    if(lib2>0){
                        ...
                        if(score > best_val){ best_val = score; best_non_suicide = d; }
                    }
                }
                if(best_non_suicide != -1) pick = best_non_suicide; /* switch to safer move */
            }
        }
