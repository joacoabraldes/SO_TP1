...
static bool sim_any_player_has_move(int *board, int width, int height, sim_player_t *players, int player_count) {
    for (int i = 0; i < player_count; i++) {
        ...
        for (int d = 0; d < 8; d++) {
            if (sim_is_valid_move(board, width, height, players, player_count, i, d)) {
                return true;
            }
        }
    }
...
static int sim_pick_policy_move(int *board, int width, int height, sim_player_t *players, int player_count, int pid) {
    ...
    for (int i = 0; i < valid_count; i++) {
        ...
        /* temporarily mark to evaluate liberties */
    ...
    while (1) {
        ...
        if (free_cells >= opening_threshold) {
            ...
            for (int i = 0; i < valid_count; i++) {
                ...
                int oldx = players_snapshot[my_index].x;
                int oldy = players_snapshot[my_index].y;
                board_snapshot[ty * gwidth + tx] = -(my_index + 1);
                players_snapshot[my_index].x = tx;
                players_snapshot[my_index].y = ty;
                int lib = sim_count_liberties(board_snapshot, gwidth, gheight, players_snapshot, my_index);
                players_snapshot[my_index].x = oldx;
                players_snapshot[my_index].y = oldy;
                board_snapshot[ty * gwidth + tx] = saved;
                if (lib == 0) {
                    val -= 1000.0; /* large penalty to avoid suicide */
                }
        ...
        /* Anti-suicide final check: if selected move leaves zero liberties try to pick alternative */
        {
            ...
            board_snapshot[ty * gwidth + tx] = saved;
            if (lib == 0) {
                int best_non_suicide = -1;
                double best_val = -DBL_MAX;
                for (int i = 0; i < valid_count; i++) {
                    int d = valid_dirs[i];
                    int tx2, ty2;
                    target_from_dir(gx, gy, d, &tx2, &ty2);
                    int saved2 = board_snapshot[ty2 * gwidth + tx2];
                    int oldx2 = players_snapshot[my_index].x;
                    int oldy2 = players_snapshot[my_index].y;
                    board_snapshot[ty2 * gwidth + tx2] = -(my_index + 1);
                    players_snapshot[my_index].x = tx2;
                    players_snapshot[my_index].y = ty2;
                    int lib2 = sim_count_liberties(board_snapshot, gwidth, gheight, players_snapshot, my_index);
                    players_snapshot[my_index].x = oldx2;
                    players_snapshot[my_index].y = oldy2;
                ...
                if (best_non_suicide != -1) {
                    pick = best_non_suicide; /* switch to safer move */
                }
            }
        }
